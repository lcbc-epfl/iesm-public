...
b) If we wish to construct a single molecular orbital
\begin{equation}
\phi_m(\mathbf{r}_m)
\end{equation}
 (which is a one-particle molecular orbital) for a carbon atom with a spin multiplicity \textit{S=5} using the 3-21G basis set, how many coefficients and exponants coming from contracted GTOs are fixed, and which expansion coefficients will vary for the calculation of the Hartree-Fock energy?
\end{mdframed}
